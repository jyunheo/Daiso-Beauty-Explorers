...
- **`crawling.py`** : 메인 화면의 카테고리별(스킨케어, 선케어, 클렌징, 메이크업) 추천 제품을 보여주기 위해, 다이소몰의 인기 뷰티 제품 정보를 수집하는 사전 데이터 크롤링 코드
- **`daiso_recommend_data.csv`** : `crawling.py`를 통해 수집한 다이소 뷰티 제품 정보와 실제 고객 리뷰가 저장되어 있는 원본 데이터 파일
