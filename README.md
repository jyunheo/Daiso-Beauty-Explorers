...
- **[경로 A - 기존 데이터 활용]:** 입력된 품번이 사전에 수집한 CSV 데이터셋에 존재할 경우, 즉시 평점 및 카테고리 추천 화면을 띄워줍니다.
- **[경로 B - 실시간 크롤링 엔진 가동]:** 데이터셋에 없는 새로운 품번이 입력되면, 서버 내부의 셀레니움(Selenium)이 실시간으로 해당 다이소몰 상품 페이지에 원격 접속하여 최신 리뷰를 크롤링 합니다.
