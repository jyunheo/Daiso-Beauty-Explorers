...
> 소비자가 별도의 프로그램 설치 없이 웹 링크 접속으로 제품의 품번을 입력하면 실시간 리뷰 분석을 확인할 수 있으며, 카테고리별 인기 제품을 한눈에 확인하고 추천받을 수 있는 모바일 최적화 웹 서비스입니다.
...
- **`streamlit.py`** : 소비자가 스마트폰으로 접속하는 메인 대시보드 애플리케이션 코드. 로컬 데이터에 없는 품번이 입력될 경우, 백그라운드에서 실시간으로 다이소몰을 크롤링하여 분석하는 엔진이 내장됨.
